...
export function getImageReviewProviderConfig(): ImageReviewProviderConfig {
  ...
  const baseUrl = normalizeBaseUrl(readEnv("FOG_IMAGE_REVIEW_BASE_URL") || DEFAULT_BASE_URL);
  const model = readEnv("FOG_IMAGE_REVIEW_MODEL") || DEFAULT_MODEL;
  const apiKey = readEnv("FOG_IMAGE_REVIEW_API_KEY");

  if (!apiKey) {
    throw new Error("Missing FOG_IMAGE_REVIEW_API_KEY. Set it in .env.local or the server environment.");
  }
