...
import { Clue, GameState, Monster, MonsterKind, Pickup, Player, Sigil, Survivor, Vec2 } from "./types";
...
export class GameEngine {
  ...
  private render(time: number) {
    ...
    const coneRange = CONE_RANGE * batteryFactor * (0.55 + 0.45 * flicker);
    ...
    for (const sigil of s.sigils) {
      if (sigil.progress >= 1 || sigil.active || vis(sigil.pos)) {
    ...
    const conePoly = castFlashlight(this.map, s.player.pos, s.player.angle, this.exitOpen, coneRange).map(
      (p) => ({ x: p.x - cam.x, y: p.y - cam.y }),
    );
    const nearPoly = castVisibility(this.map, s.player.pos, this.exitOpen).map(
      (p) => ({ x: p.x - cam.x, y: p.y - cam.y }),
    );
    renderDarkness(ctx, w, h, screenOrigin, conePoly, nearPoly, coneRange, brightness);
